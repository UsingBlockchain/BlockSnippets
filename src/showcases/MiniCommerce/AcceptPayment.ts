...
import { PaymentProcessing } from './Concerns/PaymentProcessing'
import { Product } from './Repositories/Product'

export class AcceptPaymentInputs extends SnippetInputs {
  @option({
    flag: 'n',
  ...
  @option({
    flag: 's',
    description: 'The product SKU for which to accept payments.',
  })
  sku: string;
}

@command({
  description: 'Accept borderless payments for your e-commerce with MiniCommerce by Using Blockchain Ltd (https://ubc.digital)',
})
export default class extends Snippet {
   ...
   *
   * @param AcceptPaymentInputs inputs
   * @return Promise<any>
   */
  @metadata
  async execute(inputs: AcceptPaymentInputs) 
  {
  ...
  protected async executeSnippet(
    ...
    const couponization: PaymentProcessing = new PaymentProcessing(
    ...
    let contractPath = this.backup.contractspath + '/AcceptPayment-' + inputs['sku'] + '.png'
    if (!fs.existsSync(contractPath)) {
      let base64 = await transactionQR.toBase64(new QRCodeSettings('M', 50)).toPromise()
      contractPath = this.backup.saveContract('AcceptPayment-' + inputs['sku'], base64)
    }
